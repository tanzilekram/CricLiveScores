package api.CricLiveScores.Service.impl;

import api.CricLiveScores.Entity.Match;
import api.CricLiveScores.Repository.MatchRepo;
...
public class MatchServiceImpl implements MatchService {
    ...
    @Override
    public List<Match> getSideBarNews() {
        List<Match> news = new ArrayList<>();
        try{
            String url = "https://www.cricbuzz.com/";

            Document document = Jsoup.connect(url).get();
            Elements liveNewsElements = document.select("cb-hm-lft");
            for (Element livenews: liveNewsElements){
                HashMap<String,String> liveNewsInfo = new LinkedHashMap<>();
                String newsHeading = livenews.select("cb-nws-hdln-ancr text-hvr-underline").text();
                String newsTime = livenews.select("cb-nws-time").text();

                Match liveNews = new Match();

                liveNews.setNewsHeading(newsHeading);
                liveNews.setNewsTime(newsTime);
                news.add(liveNews);
            }
          } catch (IOException e) {
        e.printStackTrace();
